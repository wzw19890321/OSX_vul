...
The function IGAccelVideoContextMain::process_token_AllPostProcGVA trusts the dword at offset 0x10 in the input token and
uses it as the index for a kernel memory write. (It adds it to a valid pointer which gets passed into patch_vphal_dndi_curbe
    ...
    if (token_buf != 0){
      ...
      if (tok && count++ > 10){
        ...
        *tok = 0x00288200; // AllPostProcGVA
        ...
        tok[0x10/4] = 0x12345678;  // no bounds checking
      }
    }
  }
