/*
build this dylib:
$ clang -Wall -dynamiclib -o ig_gl_BindTextures.dylib ig_video_main_BindTextures.c -framework IOKit -arch i386 -arch x86_64

load it into chrome:
$ DYLD_INSERT_LIBRARIES=./ig_gl_BindTextures.dylib /Applications/Google\ Chrome.app/Contents/MacOS/Google\ Chrome

if the bug doesn't trigger, navigate to a website which uses webgl, eg:
  http://damienmortini.me.uk/house/

BUG:
The function IGAccelGLContext::process_token_BindTextures fails to bounds check the dword at offset 0x10 of the
BindTextures token - this value is read from user/kernel shared memory and is thus completely attacker controlled.
