...
the dword at offset 0x818 of the AVCDecode token used by the IGAccelVideoContextMedia user client is used to compute an index
for a memory read without performing any bounds checking, allowing a controlled out-of-bounds read of kernel memory - no idea
if you can get this value back to userspace but here's a PoC anyway.
