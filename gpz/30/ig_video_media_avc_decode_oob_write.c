...
BUG
the dword at offset 0x900 of the AVCDecode token used by the IGAccelVideoContextMedia user client is used to compute an index
for a memory write without performing any bounds checking, allowing a controlled out-of-bounds write to kernel memory.

The value is read in IGAccelVideoContextMedia::process_token_AVCDecode.
